 ...
 *
 */
public class JavaConsistentMethodRenaming implements ITermProcessor {
	
	public String toString() {
		return this.getClass().getName();
	}
	
	public JavaConsistentMethodRenaming(String init) {
		this();
	}
	...
	public JavaConsistentMethodRenaming() {
	...
	public List<String> process(List<String> tokens, int language, int granularity, int tokenType) {
		...
		for(int i = 0; i < tokens.size(); i++) {
			...
			//
			if(isIdentifier(curToken) && nextToken.equals("(")) {
				String replacement;
				if(!varmap.containsKey(curToken)) {
					varmap.put(curToken, "methodname" + varnum++);
				}
